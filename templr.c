...
student* read_csv(char* filename) {
    ...
    if (fp == NULL) perr("Error opening file");
    ...
    if(students == NULL) perr("Error allocating memory");
    ...
    while (fgets(line, 100, fp) != NULL) {
        ...
        if(token == NULL) perr("Error parsing file");
...
*/
char compute_grade(student s) {
    if(s.marks > 100) perr("Invalid marks");
    if(s.marks >= 90) return 'S';
    if(s.marks >= 80) return 'A';
    if(s.marks >= 70) return 'B';
    if(s.marks >= 60) return 'C';
    if(s.marks >= 50) return 'D';
    if(s.marks >= 40) return 'E';
    return 'U';
}

/**
 * function to write the student details to a file. Creates file with filename = student.roll .txt
 * @param s
 * @param template
*/
void write_file(student s, char* template) {
    ...
    if (fp == NULL) perr("Error opening file for writing");
    ...
    while(*p != '\0') {
        ...
        if(*p == '}') {
            ...
            if(strcmp(trimmed, "name") == 0) fprintf(fp, "%s", s.name);
            else if(strcmp(trimmed, "rollno") == 0) fprintf(fp, "%s", s.roll);
            else if(strcmp(trimmed, "marks") == 0) fprintf(fp, "%u", s.marks);
            else if(strcmp(trimmed, "grade") == 0) fputc(s.grade, fp);
            // should invalid tags be retained? or removed? or throw an error?
            // else fprintf(fp, "{%s}", varname); // retained
            // else printf("Invalid tag: {%s}\n", varname); // removed
            else perror("Invalid tag"); // error
            markup = FALSE;
            memset(varname, 0, sizeof(varname));
            p++;
            continue;
...
int main(int argc, char** argv) {
    ...
    if (fp == NULL) perr("Error opening template file");
